...
## Identification of ICU capacity limits by Region

```{r, fig.cap = "Predicted needs of ICU beds in the 13 French Regions. The red line stands for the ICU capacity limit, the dotted line stands for the scenario with $R_0 = 2.25$, the black lines for the worst and best case scenarios ($R_0 = 3$ and $R_0 = 1.5$ respectively).", dev = "svg"}
## Run model
...
```{r}
data = outcomesR1.5[Time < as.Date("2020-04-08"), ]
outcomes1 = get_outcomes_byreg(data = data)

data = outcomesR2.25[Time < as.Date("2020-04-08"), ]
outcomes2 = get_outcomes_byreg(data = data)

data = outcomes[Time < as.Date("2020-04-08"), ]
outcomes3 = get_outcomes_byreg(data = data)

```

```{r}
DT = merge(outcomes1, outcomes2, by = "Region")
DT = merge(DT, outcomes3, by = "Region")

DT = DT[, c(1,2,7,12,3,8,13,4,9,14,5,10,15,6,11,16)]

colnames(DT)[c(2,5,8,11,14)] = "R0 1.5"
colnames(DT)[c(3,6,9,12,15)] = "R0 2.25"
colnames(DT)[c(4,7,10,13,16)] = "R0 3"

caption = "Predicted number of infected cases, severe cases, cases admitted into intensive care units, cases requiring ventilation, and deaths, from 10 March 2020 to 07 April 2020, by region, for R0 values of 1.5, 2.25, and 3."
