...
Based on the estimated number of new cases per day, we inferred different outcomes. First, we computed the number of severe cases, defined according to Guan and colleagues [@GuanNEJM]. We used their estimations of age-dependent severity risks across four age groups, which were dispatched across our 17 age groups (Table \ref{TABParameters}). We also estimated the number of cases which will require hospitalization to ICU using age-dependent risks from Guan and colleagues [@GuanNEJM] (Table \ref{TABParameters}). Using data from Yang and colleagues [@Yang-lancet], we set the proportion of cases which will require mechanical ventilation to 0.711 (not age-dependent). The number of deaths was estimated using age-dependent deaths risks (10 age groups) provided by the Italian National Institute of Health (Istituto Superiore di Sanità) (Table \ref{TABParameters}) [@death].

```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
options(knitr.kable.NA = '')
caption = "Parameters used to compute the main outcomes. \\label{TABParameters}"
mergedData = merge(severity_risk, ICU_risk, 
                   by = "AgeGroup", all = TRUE)
setnames(mergedData, old = 2:3, new = c("Severity risk","ICU risk"))
mergedData = merge(mergedData, death_risk[, risk := round(risk, 3)],
                   by = "AgeGroup", all = TRUE)
mergedData[AgeGroup == "0-4", AgeGroup := "00-04"]
mergedData[AgeGroup == "5-9", AgeGroup := "05-09"]
setorder(mergedData, AgeGroup)
mergedData[AgeGroup == "00-04", AgeGroup := "0-4"]
mergedData[AgeGroup == "05-09", AgeGroup := "5-9"]
mergedData[AgeGroup == "80P", AgeGroup := "80 and older"]
setnames(mergedData, old = c("AgeGroup", "risk"),
         new = c("Age group", "Death risk"))
